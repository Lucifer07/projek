...
$z=date('m');
$bln=(int)$z;
$sql =mysqli_query($koneksi, "SELECT max(energitotal) FROM panel5 WHERE day(tanggal)='$h' and month(tanggal)='$bln' ");
$dapat=mysqli_fetch_array($sql);
$data=$dapat['max(energitotal)'];
$sql2 =mysqli_query($koneksi, "SELECT energitotal FROM panel5 WHERE day(tanggal)='$h' and month(tanggal)='$bln' ");
$jumlah = mysqli_num_rows($sql2);
if ($jumlah>1) {
    $query="DELETE from panel5 where energitotal<'$data' and day(tanggal)=$h and month(tanggal)=$bln ";
...
<body>
    <section class="sheet padding-10mm">
        ...
        <h1>LAPORAN MONITORING PANEL5</BR><?= $tgl1?> HINGGA <?= $a?></h1>
        </center>
        <table class="table">
            <thead>
                <tr>
            ...
            <tbody>
            <?php
            $cek_data = mysqli_query($koneksi, "SELECT * FROM panel5 WHERE tanggal between '$tgl1' and '$tgl2' ORDER BY tanggal");
            $energis=[];
            $energit=[];
            $energir=[];
            $energitotal=[];
